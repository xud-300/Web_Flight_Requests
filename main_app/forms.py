...
class FlightRequestCreateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # 3. Проверка пикетов
        # 3.1 Пикеты не должны быть меньше 0, и должны быть целыми
        ...
        # 3.2 "Пикет от" не должен быть больше "Пикета до"
        ...
        # 4. Проверка дат съёмки
        today = timezone.now().date()
        # Дата съёмки не должна быть в прошлом
        ...
        # "Дата съемки от" не должна быть позже "Даты съемки до"
        ...
        if not (orthophoto or laser or panorama or overview):
            ...
            # Добавляем ошибку к полю "overview", чтобы сообщение отображалось возле одного чекбокса
...
class FlightRequestEditForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # 2. Название объекта не может оставаться пустым (если тип не "ЖД" (id=2)).
        if object_type and str(object_type.id) != "2":
            if not object_name:
                self.add_error('object_name', 'Пожалуйста, выберите название объекта.')
